...
function HomeView({ menu, go, notify, setCard, onItem }: { menu: Menu | null; go: (t: string) => void; notify: (m: string) => void; setCard: (v: boolean) => void; onItem: (i: Item) => void }) {
  ...
  const hero = items.find(i => i.id === 118) ?? items.find(i => /cascara/i.test(i.name)) ?? items.find(i => i.cat === 2) ?? items[0];
  const isCascara = !!hero && /cascara/i.test(hero.name);
  const heroName = isCascara ? { head: 'Cascara', tail: 'Orange' } : hero ? splitName(hero.name) : { head: 'Cascara', tail: 'Orange' };
  const heroImg = isCascara ? '/brand/photos/product_cascara_orange.jpg' : imgUrl(hero?.image ?? '', 1000);
  const heroStyle: CSSProperties = heroImg ? { backgroundImage: `url("${heroImg}")` } : {};
  const catName = new Map((menu?.categories ?? []).map(c => [c.id, c.name] as const));
  const usedIds = new Set<number>([hero?.id as number, ...SIGNATURES.map(s => s.id)]);
  const rail = items.filter(i => i.image && !usedIds.has(i.id)).slice(0, 8);
  return <>
    ...
    <section className="welcome"><div><p className="eyebrow">GOOD MORNING</p><h1>Welcome back, <em>Mohamed</em></h1><p className="welcome-sub">Good coffee brings better days.</p></div><button className="icon-btn" onClick={() => go('Menu')}><Search size={17} /></button></section>
    ...
    {hero && <section className="hero-product" onClick={() => onItem(hero)}><div className="hero-image" style={heroStyle}><div className="hero-copy"><span>SIGNATURE · 1718</span><h2>{heroName.head}{heroName.head && <br />}<em>{heroName.tail}</em></h2><p>{hero.recipe || 'A 1718 signature, crafted with care.'}</p><b>{egp(hero.price)}</b></div></div><button className="round-arrow"><ChevronRight /></button></section>}
    ...
    {rail.length > 0 && <><SectionTitle eyebrow="FRESH THIS WEEK" title="From the roastery" action="Full menu" onClick={() => go('Menu')} compact />
    <div className="product-scroll">{rail.map(p => <button className="product-card" key={p.id} onClick={() => onItem(p)}><div className="product-photo" style={photoBg(p.image, 360)} /><div className="product-info"><small>{catName.get(p.cat) ?? '1718'}</small><strong>{p.name}</strong><b>{egp(p.price)}</b></div><span className="plus"><Plus size={15} /></span></button>)}</div></>}
    <SectionTitle eyebrow="MEMBERS ENJOY" title="Your privileges" action="" onClick={() => go('Club')} compact />
    <div className="perks">
      <button className="perk" onClick={() => go('Rewards')}><Gift /><b>Birthday drink</b><span>On us, every year</span></button>
      <button className="perk" onClick={() => go('Rewards')}><Sparkles /><b>Early access</b><span>New drops first</span></button>
      <button className="perk" onClick={() => go('Club')}><Star /><b>Double points</b><span>Every Tuesday</span></button>
    </div>
    ...
    <SectionTitle eyebrow="VISIT US" title="The Roastery" action="" onClick={() => go('Menu')} compact />
    <div className="location-card home"><MapPin /><div><small>Flagship · Madinaty</small><strong>1718 Coffee &amp; Roastery</strong><span>Open today · 8:00 AM – 12:00 AM</span></div><ChevronRight /></div>
    <div className="home-footer"><span>17<sup>18</sup></span><p>MORE THAN COFFEE</p></div>
  </>;
}
